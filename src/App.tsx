...
function App() {
  ...
  const filteredProjects = useMemo(() => {
    const filtered = filterProjects(transformedProjects, filters);
    return sortProjects(filtered, sortBy);
  }, [transformedProjects, filters, sortBy]);

  // 카테고리 목록
  const categories = useMemo(() => {
    return Array.from(new Set(transformedProjects.map(p => p.category))).sort();
  }, [transformedProjects]);

  // 추천 프로젝트 (높은 품질)
  const featuredProjects = useMemo(() => {
    return getHighQualityProjects(transformedProjects, 3);
  }, [transformedProjects]);
      ...
      <main className="container mx-auto px-4 py-8">
        {viewMode === 'projects' ? (
          ...
          <div>
            ...
            <StatsOverview projects={transformedProjects} />
          </div>
        )}
      </main>
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="text-center">
            ...
            <div className="mt-4 flex justify-center gap-4 text-sm text-gray-500">
              <span>총 {transformedProjects.length}개 프로젝트</span>
              <span>•</span>
              <span>{transformedProjects.filter(p => p.is_live).length}개 라이브 사이트</span>
              <span>•</span>
              <span>평균 품질 {Math.round(transformedProjects.reduce((sum, p) => sum + (p.quality_score || 0), 0) / transformedProjects.length)}점</span>
            </div>
          </div>
        </div>
      </footer>
